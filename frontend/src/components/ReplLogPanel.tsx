    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-1 space-y-0.5">
          {replSteps.length === 0 && (
            <p className="text-xs text-terminal-amber-dim font-mono">
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {replSteps.length === 0 && (
          <p className="text-xs text-terminal-amber-dim font-mono text-center mt-10">
        ...
        {replSteps.map((step, i) => (
          <div key={i} className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono text-terminal-amber-bright text-glow">
                ITERATION_{step.iteration}
              </span>
              {step.has_answer && (
                <span className="text-xs font-mono t-border px-2 py-0.5 text-terminal-amber-bright bg-terminal-amber-faint text-glow">
                  SUBMIT
                </span>
              )}
            </div>
            <div className="t-border p-2 bg-terminal-dark overflow-x-auto">
              <pre className="text-xs text-terminal-amber whitespace-pre-wrap font-mono">
                {step.code}
              </pre>
            </div>
            {step.output && (
              <div className="p-2 bg-terminal-dark overflow-x-auto" style={{ borderLeft: '2px solid #8a6a10' }}>
                <pre className="text-xs text-terminal-amber-dim whitespace-pre-wrap font-mono">
                  {step.output}
                </pre>
              </div>
            )}
